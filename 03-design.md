...
4. Прозрачность для пользовательского кода: переключение между семантиками «не реже одного раза» и строго однократной выполняется конфигурационным параметром `spark.yt.streaming.transactional`, без модификации описания потокового запроса.
...
Часть необходимой функциональности располагается во внутренних классах Spark, не предусматривающих штатного расширения. Для воздействия на эти классы в SPYT применяется механизм инструментации байт-кода, реализованный в модуле `spark-patch`. Главный компонент модуля — Java-агент [`SparkPatchAgent`](https://github.com/ytsaurus/ytsaurus-spyt/blob/main/spark-patch/src/main/java/tech/ytsaurus/spyt/patch/SparkPatchAgent.java), регистрируемый через параметр `-javaagent` JVM драйвера и исполнителей. Агент сканирует classpath в поиске классов-декораторов, помеченных аннотациями [`@PatchSource`](https://github.com/ytsaurus/ytsaurus-spyt/blob/main/spark-patch/src/main/java/tech/ytsaurus/spyt/patch/annotations/PatchSource.java) и [`@Decorate`](https://github.com/ytsaurus/ytsaurus-spyt/blob/main/spark-patch/src/main/java/tech/ytsaurus/spyt/patch/annotations/Decorate.java), и подменяет реализации соответствующих методов в исходных классах Spark, сохраняя оригинальное поведение и оборачивая его в пользовательский код.
...
В реализации источника соответствие между партициями RDD и таблетами очереди установлено явно. В классе [`YtQueueRange`](https://github.com/ytsaurus/ytsaurus-spyt/blob/main/data-source-extended/src/main/scala/tech/ytsaurus/spyt/streaming/YtQueueRange.scala) индекс Spark-партиции переопределён равным индексу таблета:
...
В методе [`YtQueueRDD.getPartitions`](https://github.com/ytsaurus/ytsaurus-spyt/blob/main/data-source-extended/src/main/scala/tech/ytsaurus/spyt/streaming/YtQueueRDD.scala) партиции возвращаются отсортированными по `tabletIndex`, что обеспечивает стабильный порядок и взаимно однозначное соответствие между партициями RDD и таблетами очереди.
...
| Семантика при произвольных преобразованиях                      | «не реже одного раза»       | строго однократная                  | строго однократная                                       |
| Семантика при stateless-преобразованиях с малым объёмом таблицы | строго однократная          | строго однократная                  | строго однократная                                       |
