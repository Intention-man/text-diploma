...
3. На выбранном исполнителе открывается tablet-транзакция через локальный rpc-прокси.
...
Сильная сторона решения — простота реализации, локализация изменений в одном модуле SPYT и отсутствие изменений в сторонних проектах. Решение опирается на штатный механизм tablet-транзакций, описанный в главе 2, и не требует расширения клиентской библиотеки YT.
...
Модуль `yt-wrapper` содержит вспомогательные утилиты для работы с tablet-транзакциями YTsaurus. В нём вводится утилита, оборачивающая операцию `attachTransaction`, расширяющая клиентскую библиотеку YT.
...
> **TODO:** Формализованное описание протокола атомарной фиксации батча в решении 2: формальная нотация шагов, инварианты после каждого шага, разбор поведения при сбое на каждом шаге (открытие транзакции, broadcast, modifyRows на исполнителе, потеря связи с rpc-прокси, advanceConsumer на драйвере, commit), взаимодействие с offset/commit-логом Spark при штатной отмене и при переисполнении батча. Раздел дописывается после получения работающего прототипа решения 2 с целью уточнения деталей поведения, наблюдаемых в реализации.
...
Конфигурация с общим пулом rpc-прокси (`spark.ytsaurus.rpc.job.proxy.enabled=false`) переносит точку отказа из job-proxy исполнителей в общий пул. Все запросы транзакции — `createTransaction`, `pingTransaction`, `modifyRows` с каждого исполнителя, `advanceConsumer`, `commit` — обслуживаются одним rpc-прокси, на котором открыта транзакция. Отказ этого прокси в течение жизни транзакции приводит к её аварийному завершению независимо от состояния запросов записи на исполнителях.

Принятые меры смягчения. Во-первых, аварийное завершение транзакции не нарушает корректности: незафиксированная транзакция не оставляет следа ни в выходной таблице, ни в состоянии потребителя, и Spark переисполняет батч штатным образом. Во-вторых, общий пул rpc-прокси YTsaurus состоит из множества равноправных экземпляров с собственным резервированием на уровне инфраструктуры YTsaurus, и среднее время между отказами одного прокси из пула существенно превосходит соответствующую величину для одиночного процесса исполнителя. В-третьих, на уровне SPYT не вводится дополнительной логики восстановления соединения: повторное открытие транзакции выполняется на следующем батче после переисполнения текущего.

### 3.7.3 Соотношение времени жизни tablet-транзакции и длительности батча

Tablet-транзакция YTsaurus имеет ограниченное время жизни, продлеваемое периодическими сообщениями `pingTransaction` (§2.3.3). Если обработка батча превышает таймаут транзакции и пинги не отправляются вовремя, прокси отменяет транзакцию автоматически, что наблюдается на драйвере как ошибка «No such transaction» при попытке `commit`.
...
Принятые меры смягчения. Во-первых, исполнитель не пытается восстановить соединение или повторно присоединиться к транзакции: исключение, выброшенное из замыкания `foreachPartition`, передаётся в драйвер штатным механизмом Spark и обрабатывается как сбой задачи записи. Во-вторых, драйвер при первом сбое любой задачи записи отменяет транзакцию через `abort` и не пытается фиксировать частично завершённую запись. В-третьих, переисполнение батча выполняется штатным механизмом Spark Structured Streaming с тем же набором смещений источника, поскольку `Source.commit` не успел выполнить продвижение смещения. Корректность сохраняется в силу того, что отменённая tablet-транзакция не оставляет видимых строк в выходной таблице (§2.2.1).

### 3.7.5 Сводка компромиссов

Принятые архитектурные компромиссы решения 2 формулируются следующим образом.
